...
简单说，就是把你**真正看完**的内容同步到豆瓣，不刷屏、不重复、不误报。

<h3>🎬 电影</h3>

一部电影你看到 90% 以上（默认值，可以自己调），就算"看过了"→ 豆瓣标记「看过」。
不到 90% 的随手点开？跳过，不上豆瓣报信。

<h3>📺 电视剧</h3>

比如你追一部 30 集的剧：

- **看了第 1 集** → 豆瓣标记 **「在看」**（告诉别人：这剧我开追了）
- **看了第 2 ~ 29 集** → **啥也不干**（没必要每看一集都去豆瓣刷一下）
- **看了第 30 集大结局** → 豆瓣标记 **「看过」**（告诉别人：追完了）

**如果装这个工具之前你就已经把剧看完了：**
系统发现你 30 集全看了，直接全部标「看过」，不会傻乎乎去标「在看」。

**如果同一集下次同步又扫到：**
系统会直接跳过，不会再去豆瓣重复标记——说一次就够了，不会天天跑去喊"我看过了"。

<h3>🧠 如果飞牛里分不清哪季是哪季</h3>

有些剧飞牛里没标"第几季"，所有剧集混在一起。这时候系统会自动分析集数编号，比如第 1~12 集是一组、第 13~24 集又是一组——数值一回头就是新的一季。这样还是能正确匹配豆瓣上分季的条目，不会把《权力的游戏》S1 和 S2 混成一锅粥。
